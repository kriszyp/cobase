  ...
  return class extends Persistable.as(VArray) {
        ...
        if (!deleted) {
          ...
          try {
            ...
            try {
              ...
              console.warn('Error retrieving value needing to be indexed', error)
            }
          }
    ...
    static commitOperations() {
      ...
      operationsToCommit.push({
        ...
        value: lastIndexedVersion
      })
      console.log('commit operations', this.name, 'at', lastIndexedVersion)
      //if (operationsToCommit.length > 200 || operationsToCommit.byteCount > 100000) {
      // large number, commit asynchronously
      this.sendUpdates()
      return when(Source.writeCompletion, () => this.getDb().batch(operationsToCommit))
    }
    static sendUpdates() {
